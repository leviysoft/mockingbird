...
const methodName = 'market_data.OTCMarketDataService/PricesClient'
...
export default function() {
  ...
  const methodDescriptionData = {
    id: "client-streaming-method-description",
    description: "k6 testing",
    service: serviceName,
    methodName: methodName,
    connectionType: "CLIENT_STREAMING",
    proxyUrl: null,
  ...
  const stubData = {
    methodDescriptionId: "client-streaming-method-description",
    scope: "countdown",
    times: 1,
    name: "client streaming v4 countdown stub",
    response: {
      "data": {
        "code": "OK",
  ...
  stream.on('data', (data) => {
    ...
    check(data, {
       'response stream element - validation is OK': (d) => d &&
         d.instrumentId === grpcReq[0].instrument_id && d.trackingId === grpcReq[0].instrument_id_kind,
    })
  })
